...
_ini = int(get_parameter('ini','SELIC','pos_ini'))
_fim = int(get_parameter('ini','SELIC','pos_fim'))
...
def get_selic(self,_host, _database, _user, _password):
    try:
        ## Conexao
        ...
        rv = connect_mysql.select_advanced(sql_query, ('TransacoesRFIndexador', 'SELIC'))
        ...
        if len(rv)>0:
            ...
            for objsql in json_sql:
                ...
                # arruma as datas
                i_strdia = '0' + str(data.day)
                i_strdia = i_strdia[-2:]
                i_strmes = '0' + str(data.month)
                i_strmes = i_strmes[-2:]
                i_strano = str(data.year)
                f_strdia = '0' + str(datetime.date.today().day)
                f_strdia = f_strdia[-2:]
                f_strmes = '0' + str(datetime.date.today().month)
                f_strmes = f_strmes[-2:]
                ...
                data = "aba=4&dataInicial=" + i_strdia + "%2F" + i_strmes + "%2F" + i_strano + "&dataFinal=" + f_strdia + "%2F" + f_strmes + "%2F" + f_strano + "&valorCorrecao=1%2C00"
        ...
        ucount = 0
        rowsupdated = 0
        result = '0 Registros Selic Alterados.'
        ...
        for obj in lista:
            ...
            result = str(ucount)+" Registro(s) Selic Alterado(s)"
        ...
        logging.error("Erro no getselic: {0}\n".format(str(e)), exc_info=True)
        return "Houve um erro e os dados da SELIC nao foram atualizados"
    ### FIM
